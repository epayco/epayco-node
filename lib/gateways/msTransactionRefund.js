/**
 * Gateway for reversing/refunding an already-created transaction against the
 * "ms-transaction" microservice's generic transaction-operations API
 * (PUT /transaction/refund/{refPayco}, tag "Operaciones sobre transacciones
 * creadas" -- see
...
function login(apiKey, privateKey) {
    ...
    return fetch(BASE_URL_MS_AUTHENTICATION + '/api/v1/oauth/login', {
        ...
        .then(function (json) {
            return json && json.access_token && json.access_token.accessToken;
        });
}
...
/**
 * Reverse/refund an existing transaction by refPayco.
 *
