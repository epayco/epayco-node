...
function buildBody(epaycoCtx, options) {
    ...
    var body = {
        ...
        country: country,
        ip: options.ip,
        // Falls back to url_confirmation because the README's own documented
        // safetypay.create() example never sets url_response (only
        // url_confirmation) -- the legacy apify flow tolerates that, but
        // ms-transaction rejects a null responseUrl outright.
 ...
 * no transaction record was ever created), legacy's own comparable failure
 * responses do NOT return a rich transaction-shaped `data` object -- see
 * msTransactionCash.js's buildLegacyErrorShape for the full rationale,
 * verified empirically the same way for SafetyPay (a bad end_date rejection
 * returns legacy's thin `{totalErrors, errors:[{codError, errorMessage}]}`
 * validation shape).
 *
 * @param {Object} raw ms-transaction response body ({success, message, data})
