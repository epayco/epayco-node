...
/**
 * Map a successful ms-transaction TDC response into (approximately) the
 * shape the legacy /payment/v1/charge/create endpoint returns today (see
 ...
 * as every other gateway's mapToLegacyShape.
 *
 * NOT a fully verified field-by-field mapping (see this file's header note):
 * only one real "Rechazada" and one real "Pendiente" (in-progress 3DS)
 * response have been paired against this mapping so far, no "Aceptada".
 *
 * `country_card`/`ind_pais` (the card's issuing country, e.g. "PE" in the
 * one real legacy response observed) has NO verified equivalent in the
 * ms-transaction response -- left empty rather than guessed.
 *
 * `threeDsAuthentication` is included as an ADDITIONAL field with no legacy
 * equivalent at all (3DS is new behavior for this SDK) -- callers that need
 * to react to an in-progress 3DS challenge/frictionless flow (see
 * `estado`/`cod_error` "189" observed in the one real 3DS-in-progress
 * response) can read it directly.
 *
 * @param {Object} raw ms-transaction response body ({success, message, data})
 * @param {Object} options the original caller-supplied options
 * @return {Object} approximately legacy-shaped response
 */
function mapToLegacyShape(raw, options) {
    ...
    return {
        status: success,
        success: success,
        type: success ? 'Create payment' : 'Error',
        message: success ? raw.message : extractErrorMessage(raw),
        data: {
            ...
            threeDsAuthentication: providerData.threeDsAuthentication
        },
        object: 'payment'
    };
}
