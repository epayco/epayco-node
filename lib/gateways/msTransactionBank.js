...
var BASE_URL_MS_TRANSACTION_AUTH = process.env.BASE_URL_MS_TRANSACTION_AUTH || 'https://apify-green.epayco.co';
...
/**
 * Build the extrasEpayco object: the integrator's `extrasEpayco` values win,
 * and extra5 falls back to the "P44" marker only when it was not sent (or
 * was sent empty).
 *
 * @param {Object} options
 * @return {Object}
...
/**
 * When ms-transaction rejects a request outright (raw.success === false),
 * legacy's own comparable failure responses do NOT return a rich
 * transaction-shaped `data` object -- see msTransactionCash.js's
 * buildLegacyErrorShape for the full rationale (verified empirically the
 * same way for PSE: a duplicate-invoice rejection returns legacy's thin
 * `{totalerrores, errores:[{codError, errorMessage}]}` validation shape, a
 * harder failure returns no `data` key at all).
 *
 * @param {Object} raw ms-transaction response body ({success, message, data})
 * @param {String} [plainAction] last_action for the data-less shape; real
 *        paired example for PSE: 'Ingresar pago debito Pse'.
 * @return {Object} legacy-shaped error response (no fabricated transaction data)
 */
function buildLegacyErrorShape(raw, plainAction) {
    var data = raw && raw.data;
    ...
    if (hasStructuredErrors) {
        return {
            ...
            text_response: extractErrorMessage(raw),
            last_action: 'validation transaction',
            data: {
                totalerrores: data.errors.length,
                errores: data.errors.map(function (e) {
    ...
    return {
        ...
        title_response: 'Error',
        text_response: extractErrorMessage(raw),
        last_action: plainAction || 'validation transaction'
    };
}
 ...
 *
 * When ms-transaction rejects the request outright (raw.success === false --
 * no transaction record was ever created server-side), this returns legacy's
 * own thinner failure shape instead of a hybrid data object -- see
 * buildLegacyErrorShape (msTransactionCash.js has the full rationale,
 * verified empirically the same way for this gateway).
 *
 * @param {Object} raw ms-transaction response body ({success, message, data})
...
function mapToLegacyShape(raw) {
    if (!raw.success) {
        return buildLegacyErrorShape(raw, 'Ingresar pago debito Pse');
    }
    var data = raw.data || {};
...
/**
 * Map a successful ms-transaction PSE-banks response into the exact shape
 * the legacy /restpagos/pse/bancos.json endpoint returns (see
 * lib/resources/bank.js#getBanks) -- verified against a real paired
 * pre-prod call to both backends: `data` (the bankCode/bankName list) came
 * back byte-for-byte identical between the two (same 3 entries, same
 * codes/names), so it is passed through unchanged here.
 *
 * `enpruebas` has NO equivalent field in the ms-transaction response at all
 * (unlike `mapToLegacyShape` above, there is nothing in `raw` to read it
 * from) -- it is instead derived from `epaycoCtx.test`, the same value this
 * SDK itself just sent to identify the merchant/session as test-mode, using
 * the `1`=pruebas/`2`=producción encoding confirmed empirically for this
 * same field on both bank.create()'s legacy response and both flows'
 * bank.get() response (see SDK-1355 QA notes) -- not guessed, but inferred
 * from a value already known locally rather than echoed back by this
 * particular endpoint.
 *
 * `title_response`/`last_action` are fixed constants copied verbatim from
 * the real legacy response (`"Ok"`, `"Query Bancos"`) -- this legacy
 * endpoint's success shape doesn't follow the `"SUCCESS"`/`"Crear pin X"`
 * conventions the create() endpoints use.
 *
 * @param {Object} raw ms-transaction response body ({success, message, data})
 * @param {Object} epaycoCtx the Epayco instance (apiKey/privateKey/test)
 * @return {Object} legacy-shaped response
 */
function mapToLegacyShapeBanks(raw, epaycoCtx) {
    return {
        success: !!raw.success,
        title_response: raw.success ? 'Ok' : 'Error',
        text_response: raw.message,
        last_action: 'Query Bancos',
        data: raw.data,
        enpruebas: epaycoCtx.test === 'TRUE' ? 1 : 2
    };
}

/**
 * List available PSE banks against the ms-transaction generic PSE-banks
 * endpoint. Endpoint and shape (GET .../payment/api/v1/pse/banks/{apiKey})
 * are NOT documented in SDK-1355's Jira ticket (which only covers create +
 * query) -- found instead in the already-shipped Python SDK's own
 * ms-transaction migration (epaycosdk/gateways/ms_transaction.py,
 * PSE_BANKS_URL/pse_banks()) and confirmed here against real pre-prod
 * using this SDK's own existing Basic-login auth (no need for the newer
 * client_credentials auth Python's pse_banks() uses for its own token --
 * the Bearer token from login() above, already used by create()/
 * getTransaction(), works against this endpoint too).
 *
 * @param {Object} epaycoCtx the Epayco instance (apiKey/privateKey/test)
 * @return {Promise<Object>} legacy-shaped response (see mapToLegacyShapeBanks)
 */
function getBanks(epaycoCtx) {
    return login(epaycoCtx.apiKey, epaycoCtx.privateKey)
        .then(function (token) {
            return fetch(BASE_URL_MS_TRANSACTION + '/payment/api/v1/pse/banks/' + encodeURIComponent(epaycoCtx.apiKey), {
                method: 'GET',
                timeout: REQUEST_TIMEOUT_MS,
                headers: {
        ...
        .then(function (res) { return res.json(); })
        .then(function (raw) { return mapToLegacyShapeBanks(raw, epaycoCtx); })
        .catch(function (err) {
            console.error('msTransactionBank getBanks error:', err.message);
            return { error: err.message };
        });
}
...
module.exports = {
    ...
    mapToLegacyShapeBanks: mapToLegacyShapeBanks,
