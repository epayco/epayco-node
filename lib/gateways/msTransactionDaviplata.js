 ...
 * verified empirically the same way for Daviplata (a "Máximo de intentos
 * permitidos" rejection returns legacy's thin `{totalErrors, errors}`
 * validation shape, an insert-failure returns the same shape with a real
 * `errors` entry instead).
 *
 * @param {Object} raw ms-transaction response body ({success, message, data})
