...
// Constants
const BASE_URL = getenv("BASE_URL_SDK") ? getenv("BASE_URL_SDK") : "https://api.secure.payco.co",
    BASE_URL_SECURE = getenv("SECURE_URL_SDK") ? getenv("SECURE_URL_SDK") :"https://secure.payco.co",
    ENTORNO = getenv("ENTORNO_SDK") ? getenv("ENTORNO_SDK") : "/restpagos",
    BASE_URL_APIFY = getenv("BASE_URL_APIFY") ? getenv("BASE_URL_APIFY") :"https://apify.epayco.co"
...
Resource.prototype.request = async function (method, url, data, sw, cashData, card, apify) {
    ...
    if(apify){
        ...
        url = BASE_URL_SECURE + ENTORNO + url;
    } else {
        url = BASE_URL + url;
    }
