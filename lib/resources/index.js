...
Resource.prototype.request =async function(method, url, data, sw, cashData, sp,card)  {
    ...
    const token_bearer='Bearer '+auth['bearer_token'];
        ...
        if (sw) {
            ...
            url_ = url;
            url = getEpaycoBasecureBaseUrl(BASE_URL)+ url_;
        }
    ...
    function authent(apiKey, privateKey) {
        ...
        return fetch(`${getEpaycoBasecureBaseUrl(BASE_URL)}/v1/auth/login`, {
            method: 'post',
    ...
    function getEpaycoBasecureBaseUrl(url){
        try {
            const baseUrl = getenv('EPAYCO_NODE_SDK_ENV');

            if(false === baseUrl || 'prod'=== baseUrl){
                return url;
            }
    
            if('stage' === baseUrl || 'test'=== baseUrl){
